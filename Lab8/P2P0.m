%
function [errL2, errH1, uh,ph] = P2P0(xv,yv,vertices,edges,endpoints,boundary,boundedges);
%
% ---------------------------------------------------------------
% FEM per k=2
...
for i=1:6
    for q=1:Nq
        phihq(i,q) = phih2_p2p0(i,xhq(q),yhq(q));
    end
end
%
...
for i=1:6
    for q=1:Nq
        [gx,gy] = gradhphih2_p2p0(i,xhq(q),yhq(q));
        gphihqx(i,q) = gx;
        gphihqy(i,q) = gy;
    end
end
