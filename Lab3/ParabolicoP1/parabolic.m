function errH = parabolic(hmax, t_size, theta);
%
...
disp(' --- Errors --- ');
uI = exsol(xv,yv);
err = max(abs(uI-uht(t_size+1,:)'))/max(abs(uI));
...
% --------
errL2 = norm(uI-uht(t_size+1,:)')/norm(uI);
...
errM = sqrt( ((uI-uhtt(t_size+1,:)')'*M*(uI-uhtt(t_size+1,:)'))/(uI'*M*uI) );
disp(['  ErrM = ', num2str(errM)]);
errA = sqrt( ((uI-uhtt(t_size+1,:)')'*A*(uI-uhtt(t_size+1,:)'))/(uI'*A*uI) );
...
for iele=1:nele
    ...
    uht1 = uht(t_size+1,v1);
    uht2 = uht(t_size+1,v2);
    uht3 = uht(t_size+1,v3);
...
if (plot == 1)
    ...
    for k=1:size(vertices,1)
        ...
        uu_tmp=[uht(t_size+1,index), uht(t_size+1,index(1))];
