function b = P2load2(xv,yv,vertices,edges,boundary,boundedges,endpoints);
%
% Modifica del file dei P2 che hanno gia da Ale Russo.
% SECONDA COMPONENTE DEL CARICO
% Calcola il vettore termine noto (dato tramite carico2.m).
%
...
for iele=1:nele
    ...
    for i=1:6
        for q=1:Nq
            ...
            yq = tmp(2);
            %
            fE(i) = fE(i) + carico2(xq,yq)*phihq(i,q)*whq(q);
        end
        %
        fE(i) = 2*area*fE(i);
        %
