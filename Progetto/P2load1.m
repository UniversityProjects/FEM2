function b = P2load1(xv,yv,vertices,edges,boundary,boundedges,endpoints);
%
% Modifica del file dei P2 che hanno gia da Ale Russo.
% PRIMA COMPONENTE DEL CARICO
% Calcola il vettore termine noto (dato tramite carico1.m).
%
...
for iele=1:nele
    ...
    for i=1:6
        for q=1:Nq
            ...
            yq = tmp(2);
            %
            fE(i) = fE(i) + carico1(xq,yq)*phihq(i,q)*whq(q);
        end
        %
        fE(i) = 2*area*fE(i);
        %
