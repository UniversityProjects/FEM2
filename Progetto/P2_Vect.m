        ...
        for i=1:12
            for q=1:Nq
                if j < 6.5         % prime sei funzioni di base
                   tmpJ = JFIT(1,1:2)*[gphihqx(j,q);gphihqy(j,q)];
                elseif j > 6.5   % seconde sei funzioni di base
                    tmpJ = JFIT(2,1:2)*[gphihqx(j-6,q);gphihqy(j-6,q)];
                end
                if i < 6.5         % prime sei funzioni di base
                    tmpI = JFIT(1,1:2)*[gphihqx(i,q);gphihqy(i,q)];
                elseif i > 6.5   % seconde sei funzioni di base
                    tmpI = JFIT(2,1:2)*[gphihqx(i-6,q); gphihqy(i-6,q)];
                end
                BE(i,j) = lambda*tmpI*tmpJ*whq(q) + BE(i,j);
            end
