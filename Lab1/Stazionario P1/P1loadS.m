function bh = P1loadS(xv,yv,vertices,edges,boundary,endpoints);
%
...
% ... vettore termine noto. E' utilizzato per lo stazionario, non
% ... ha una entrata t per il tempo.
%
area = 0;
%
...
for iele=1:nele
    ...
    %
    % trapezi 
    fhT_t = T/3 * [carico(x1,y1) carico(x2,y2) carico(x3,y3)]';
    %
    % baricentro
    fhT_b = T/3 * carico(xb,yb) * [1 1 1]';
    %
    ...
    M3y = (y1+y2)/2;
    %
    fhT_2 = T/6*[carico(M2x,M2y)+carico(M3x,M3y)
                 carico(M1x,M1y)+carico(M3x,M3y)
                 carico(M1x,M1y)+carico(M2x,M2y)];
    %
